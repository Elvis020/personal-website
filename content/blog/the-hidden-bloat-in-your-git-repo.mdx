...
To actually remove these files from history, you have several options. Git officially recommends `git filter-repo` as the modern replacement for `filter-branch`.
...
git filter-repo --path path/to/large/file1 --path path/to/large/file2 --invert-paths
```
